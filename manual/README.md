# YABook Handbook

O YABook Handbook é a documentação humana e a fonte normativa do ecossistema
YABook. A YABook Skill usa estas regras para orquestrar o Método YA LABS.

Use este índice para encontrar o padrão certo sem ler o repositório inteiro.

## Se você quer...

| Intenção | Leia |
| --- | --- |
| Entender como usar o YABook em um projeto | [Manual de uso](guias/manual-de-uso.md) |
| Consultar os padrões mais usados rapidamente | [Padrões rápidos](padroes/padroes-rapidos.md) |
| Criar issue, branch, commit, PR ou release | [Fluxo de trabalho com GitHub](processos/fluxo-de-trabalho-github.md) |
| Conduzir um projeto da descoberta até release | [Condução de projetos](processos/conducao-de-projetos.md) |
| Consultar capacidades planejadas para o YABook | [Planejamento](../produto/planejamento/README.md) |
| Criar ou expandir um projeto com a skill | [Criar e expandir projetos com YABook](guias/criar-e-expandir-projetos-com-yabook.md) |
| Usar IA sem perder padrão e rastreabilidade | [Uso de IA](guias/uso-de-ia.md) |
| Organizar documentação técnica | [Documentação técnica](guias/documentacao-tecnica.md) |
| Criar um projeto novo com base YA LABS | [Template base de projeto](modelos/projeto/README.md) |
| Usar comandos YABook com IA | [Manual de uso](guias/manual-de-uso.md) |
| Verificar ou sincronizar a skill instalada | [Manual de uso](guias/manual-de-uso.md#como-sincronizar-a-skill) |
| Entender como a skill YABook funciona | [Guia técnico da skill YABook](guias/skill-yabook.md) |

## Templates úteis

- [AGENTS.md de projeto](modelos/projeto/AGENTS.md)
- [README de projeto](modelos/projeto/README.md)
- [Guia de consulta da documentação](modelos/projeto/docs/guia-da-documentacao.md)
- [Guia de documentação para IA](modelos/projeto/docs/guia-de-documentacao-para-ia.md)
- [Template de ADR](modelos/projeto/docs/adrs/000-template.md)
- [Template de prova técnica](modelos/projeto/docs/prototipos/prova-tecnica-template.md)
- [Critérios de pronto](modelos/projeto/docs/release/criterios-pronto.md)

## Limite do YABook

O YABook guarda padrões organizacionais. Documentação específica de produto deve ficar no repositório do próprio projeto.

Não coloque aqui endpoints reais, arquitetura de uma aplicação, variáveis de ambiente, deploy ou roadmap exclusivo de produto.
